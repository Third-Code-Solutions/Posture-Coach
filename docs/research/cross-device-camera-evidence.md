...
| Effective portrait frame  | [`components/coach/CoachApp.tsx`](../../components/coach/CoachApp.tsx) detects a compact landscape camera stream, rotates it into a bounded portrait HTML canvas for preview, and copies accepted inference frames into a separate bounded snapshot.     | Preview and inference preserve the same orientation and aspect ratio without asynchronous bitmap capture mutating the displayed canvas. A CSS-only rotation would require equally careful landmark remapping. |
...
| Backpressure and transfer | [`src/vision/worker-client.ts`](../../src/vision/worker-client.ts) permits one frame in flight, transfers an `ImageBitmap` when worker Canvas2D exists, and otherwise transfers a bounded RGBA `ArrayBuffer` to the worker. Rejected bitmaps are closed. | Keep one-frame-in-flight behavior. It bounds latency and memory instead of allowing a queue of stale body frames. The page readback is compatibility work; model inference remains off the main thread.       |
...
| Architecture contract     | [`docs/architecture.md`](../architecture.md#runtime-flow) documents the transferable `ImageBitmap` and RGBA-buffer worker routes plus local-only model/WASM loading.                                                                                     | Keep browser adapters, worker inference, domain evaluation, and privacy policy separate. Do not introduce a server frame endpoint merely to solve device compatibility.                                       |
